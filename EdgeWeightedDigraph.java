...
public class EdgeWeightedDigraph {
     ...
     */
    public EdgeWeightedDigraph(int choice, int numbOfVert, LinkedList<Node> adList) {
        this(numbOfVert);
           
        if(choice == 0) {
            for (int i = 0; i < adList.size(); i++) {
                Node currentNode = adList.get(i).next;

                if(currentNode == null)
                    continue;
                while(currentNode.next != null) {
                    int v = currentNode.e.start()-1;
                    int w = currentNode.e.end()-1;
                    int miles = currentNode.e.miles();
                    DirectedEdge e = new DirectedEdge(v, w, miles, 0);
                    addEdge(e); 
                    DirectedEdge d = new DirectedEdge(w, v, miles, 0);
                    addEdge(d);
        ...
        if(choice == 1) {
            for (int i = 0; i < adList.size(); i++) {
                ...
                while(currentNode.next != null) {
                    int v = currentNode.e.start()-1;
                    int w = currentNode.e.end()-1;
                    double cost = currentNode.e.cost();
                    DirectedEdge e = new DirectedEdge(v, w, 0, cost);
                    addEdge(e);
                    DirectedEdge d = new DirectedEdge(w, v, 0, cost);
                    addEdge(d);
                    currentNode = currentNode.next;
                }
                int v = currentNode.e.start()-1;
